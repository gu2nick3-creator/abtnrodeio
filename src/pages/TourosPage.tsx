...
const TourosPage = () => {
  ...
  const filtered = useMemo(() => {
    return data.bulls.filter((bull) =>
      `${bull.nome || ""} ${bull.tropeiro_nome || ""} ${bull.cidade || ""}`
        .toLowerCase()
      ...
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-4">
          ...
          <p className="text-muted-foreground mb-8">
            Busca por nome, tropeiro ou cidade.
          </p>
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filtered.map((bull) => (
                <Link
                  key={bull.id}
                  to={`/touros/${bull.id}`}
                  className="bg-card border border-gold rounded-xl overflow-hidden hover:shadow-gold transition-shadow"
                >
                  <div className="h-60 bg-muted">
                    <img
                      src={bull.foto_url || "/placeholder.svg"}
                      alt={bull.nome}
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-muted-foreground text-sm mt-2">
                      {bull.tropeiro_nome || "Sem tropeiro"}
                    </p>
