...
import type { Bull, Event, GalleryItem } from "@/data/mockData";
...
const TropeiroForm = ({ id, onClose }: { id: number | null; onClose: () => void }) => {
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (id) updateTropeiro(id, form);
        else addTropeiro(form);
        onClose();
      }}
    >
      <Field label="Nome">
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        if (id) updateBull(id, payload);
        else addBull(payload);
        onClose();
      }}
    >
      <Field label="Nome">
      ...
      <Field label="Tropeiro">
        <Select
          value={String(form.tropeiroId)}
          onChange={(e) => setForm({ ...form, tropeiroId: Number(e.target.value) })}
        >
  ...
  return (
    ...
    >
      ...
      <Field label="Touros previstos">
        <TextInput
          type="number"
          value={form.bullCount}
          onChange={(e) => setForm({ ...form, bullCount: Number(e.target.value) })}
        />
      </Field>
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        if (id) updateGalleryItem(id, payload);
    ...
    >
      ...
      <Field label="Touro">
        <Select
          value={form.bull}
          onChange={(e) => setForm({ ...form, bull: e.target.value })}
        >
      ...
      <Field label="Tropeiro">
        <Select
          value={form.tropeiro}
          onChange={(e) => setForm({ ...form, tropeiro: e.target.value })}
        >
...
const UserForm = ({ id, onClose }: { id: number | null; onClose: () => void }) => {
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (id) updateUser(id, { ...form, tropeiroId: Number(form.tropeiroId) });
        else addUser({ ...form, tropeiroId: Number(form.tropeiroId) });
        onClose();
      }}
    >
      <div className="md:col-span-2 p-4 rounded-lg border border-gold text-sm text-muted-foreground">
        Aqui o administrador cria o login do tropeiro. Esse usuário entra pela página{" "}
        <strong>/login</strong>.
      </div>
      ...
      <Field label="Tropeiro vinculado">
        <Select
          ...
          onChange={(e) => setForm({ ...form, tropeiroId: Number(e.target.value) })}
        >
          {availableTropeiros.length ? (
            availableTropeiros.map((item) => (
              <option key={item.id} value={item.id}>
                {item.name}
              </option>
            ))
