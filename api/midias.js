  ...
  try {
    ...
    if (req.method === 'POST') {
      const data = pick(req.body, [
        'tipo', 'titulo', 'categoria', 'url', 'public_id', 'formato', 'resource_type',
        'touro_id', 'tropeiro_id', 'evento_id'
      ]);
      if (!data.tipo || !data.titulo || !data.url) {
        return badRequest(res, 'Tipo, título e URL são obrigatórios');
      }
      ...
      const result = await query(
        `INSERT INTO midias (tipo, titulo, categoria, url, public_id, formato, resource_type, touro_id, tropeiro_id, evento_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          data.tipo, data.titulo, data.categoria, data.url, data.public_id, data.formato,
          data.resource_type, data.touro_id, data.tropeiro_id, data.evento_id
        ]
      );
    ...
    if (req.method === 'PUT') {
      ...
      const result = await query(
        `UPDATE midias
         SET tipo = ?, titulo = ?, categoria = ?, url = ?, public_id = ?, formato = ?, resource_type = ?,
             touro_id = ?, tropeiro_id = ?, evento_id = ?
         WHERE id = ?`,
        [
          data.tipo, data.titulo, data.categoria, data.url, data.public_id, data.formato,
          data.resource_type, data.touro_id, data.tropeiro_id, data.evento_id, id
        ]
      );
    ...
    const existing = await query('SELECT public_id, resource_type FROM midias WHERE id = ? LIMIT 1', [id]);
    const result = await query('DELETE FROM midias WHERE id = ?', [id]);
    if (!result.affectedRows) return notFound(res);
    return ok(res, { success: true, removed: existing[0] || null });
  } catch (error) {
