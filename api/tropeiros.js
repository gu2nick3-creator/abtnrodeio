...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const data = pick(req.body, ['nome', 'telefone', 'equipe', 'cidade', 'estado', 'descricao', 'instagram', 'facebook', 'foto_url', 'video_url']);
      if (!data.nome) return badRequest(res, 'Nome é obrigatório');
      const result = await query(
        `INSERT INTO tropeiros (nome, telefone, equipe, cidade, estado, descricao, instagram, facebook, foto_url, video_url)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [data.nome, data.telefone, data.equipe, data.cidade, data.estado, data.descricao, data.instagram, data.facebook, data.foto_url, data.video_url]
      );
    ...
    if (req.method === 'PUT') {
      ...
      const result = await query(
        `UPDATE tropeiros
         ...
         WHERE id = ?`,
        [data.nome, data.telefone, data.equipe, data.cidade, data.estado, data.descricao, data.instagram, data.facebook, data.foto_url, data.video_url, id]
      );
